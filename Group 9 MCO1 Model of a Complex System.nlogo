...
;  ]
  ...
  (
    list
    count passengers with [train-car = 8 and status ="die" ]
    count passengers with [train-car = 4 and status ="die" ]
    count passengers with [train-car = 0 and status ="die" ]
    count passengers with [train-car = -4 and status ="die" ]
    ; count passengers with [train-car = -8 and status ="die" ]
  )
  if count passengers with [train-car = 8 and status ="die" ] = smallest [set fewest-door 8]
  if count passengers with [train-car = 4 and status ="die" ] = smallest [set fewest-door 4]
  if count passengers with [train-car = 0 and status ="die" ] = smallest [set fewest-door 0]
  if count passengers with [train-car = -4 and status ="die" ] = smallest [set fewest-door -4]
  ; if count passengers with [door-target = -8] = smallest [set fewest-door -8]
  ...
  ask passengers [
    ...
    if status = "weighing"
    [
      ;insert queue logic that checks for the shortest queue (excluding special line for women, and seniors, and pwds
      ...
      [
        ...
        [
          ...
          ifelse pwd = "pwd"
          [ifelse chance <= 70 [set train-car -8][set train-car fewest-door]][ifelse chance <= 50[
            let rand_3 random 3
            if rand_3 = 0 [set train-car 4]
            if rand_3 = 1 [set train-car 0]
            if rand_3 = 2 [set train-car -4]][set train-car fewest-door]]
        ]
        [

          ifelse pwd = "pwd"
          [ifelse chance <= 70 [set train-car -8][set train-car fewest-door]][ifelse chance <= 70 [set train-car -8][set train-car fewest-door]]
        ]
...
This system model shows how passengers queue for a cart/wagon at an LRT station.

## HOW IT WORKS

Passengers arrive to board the next train ride. They choose a wagon to ride depending on their preference and how condensed it is.

## HOW TO USE IT

Set the sliders to number of passengers, female probability, young probability and PWD probability.


## THINGS TO NOTICE

The impact of different needs and behaviors that impact how queues are formed and populated.
...
This model could be extended to reroute traffic as to prevent overpopulation.

## NETLOGO FEATURES

Color coding of passengers as they pass enter each wagon.

## RELATED MODELS


## CREDITS AND REFERENCES
NetLogo User Community Models: (n.d.). http://ccl.northwestern.edu/netlogo/models/community/ER%20Queue%20Model%202
@#$#@#$#@
default
true
0
